					...
					const char *buf, size_t count)
{
	...
	case LOW_BATTERY:
		...
		max77865_rgb_blink(dev, leds_control.noti_delay_on, leds_control.noti_delay_off);
		max77865_rgb_set_state(&max77865_rgb->led[RED], led_dynamic_current, LED_BLINK);
		break;
	case FULLY_CHARGED:
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case LOWPOWER_CURRENT:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.current_low);
	case HIGHPOWER_CURRENT:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.current_high);
	case NOTIFICATION_RAMP_CONTROL:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.noti_ramp_control);
	case NOTIFICATION_RAMP_UP:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.noti_ramp_up);
	case NOTIFICATION_RAMP_DOWN:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.noti_ramp_down);
	case NOTIFICATION_DELAY_ON:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.noti_delay_on);
	case NOTIFICATION_DELAY_OFF:
		return snprintf(buf, RGB_BUFSIZE, "%d", leds_control.noti_delay_off);
	}
